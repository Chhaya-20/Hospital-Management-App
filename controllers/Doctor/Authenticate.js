...
exports.SignupUser = async (req, res) => {
  const { name, email, password ,experience , specialities , qualification} = req.body;
  ...
  try {
    ...
    bcrypt.hash(password, saltRounds, async (err, hash) => {
      ...
      try {
        ...
        const user = new Doctor({ name, email, password: hash ,experience , specialities , qualification});
        await user.save();
        const token = jwt.sign({ userId: user._id }, process.env.SECRET_KEY, {
          expiresIn: "1d",
        });
